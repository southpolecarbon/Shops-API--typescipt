...
function ProductList({ onAddToCart }: ProductListProps) {
  ...
  const [selectedOptions, setSelectedOptions] = useState<{ [key: string]: Record<string, string> }>({});
  ...
  const handleOptionChange = (productSku: string, attributeCode: string, valueIndex: string) => {
  ...
  const validateOptions = (product: Product): string => {
    if (product.__typename !== 'ConfigurableProduct' || !product.configurable_options) {
      return '';
    }

    const missingOptions = product.configurable_options
      .filter(option => !selectedOptions[product.sku]?.[option.attribute_code])
      ...
      <ul>
        {data.products.items.map((product: Product) => (
          <ProductItem
            ...
            onQuantityChange={(value) => handleQuantityChange(product.sku, value)}
            onOptionChange={(attributeCode, valueIndex) => handleOptionChange(product.sku, attributeCode, valueIndex)}
            onAddToCart={() => {
              const validationError = validateOptions(product);
              if (validationError) {
                setErrors(prev => ({ ...prev, [product.sku]: validationError }));
              } else {
                onAddToCart(product, quantities[product.sku] || 1, selectedOptions[product.sku]);
              }
            }}
          />
        ))}
